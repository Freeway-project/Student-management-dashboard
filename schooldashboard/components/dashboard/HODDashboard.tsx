...
import { FileCheck, CheckSquare, Clock, AlertCircle, Upload, File, X, RefreshCw, Plus, Building2 } from 'lucide-react';
...
export default function HODDashboard() {
  ...
  const isUserAssignedToTask = (task: Task, userId: string) => {
    ...
    return task.assignedTo?.some(assignee => assignee.id === userId);
  };
  ...
  const fetchTasks = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Split tasks between "my tasks" (assigned to me) and "department tasks" (all tasks in department or assigned by me)
        const myAssignedTasks = tasks.filter((task: Task) => 
          isUserAssignedToTask(task, user?.id || '')
        );
        const departmentAssignedTasks = tasks.filter((task: Task) => 
          !isUserAssignedToTask(task, user?.id || '')
        );
        
        setMyTasks(myAssignedTasks);
        setDepartmentTasks(departmentAssignedTasks);
  ...
  const handleFileUpload = async (deliverableIndex: number, file: File) => {
    try {
      ...
      const data = await response.json();
      setUploadedFiles(prev => ({ ...prev, [deliverableIndex]: data.url }));
  ...
  const handleUrlInput = (deliverableIndex: number, url: string) => {
    setUploadedFiles(prev => ({ ...prev, [deliverableIndex]: url }));
  };
    ...
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Manage your tasks and monitor your department's progress</CardDescription>
        </CardHeader>
        <CardContent>
          <Tabs defaultValue="my-tasks" className="w-full">
            <TabsList className="grid w-full grid-cols-2">
              ...
              <TabsTrigger value="department-tasks">
                Department Tasks ({departmentTasks.length})
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="department-tasks">
              {departmentTasks.length === 0 ? (
                <div className="text-center py-8">
                  <Building2 className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
                  <p className="text-muted-foreground">No tasks assigned to your department</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {departmentTasks.map(renderTaskCard)}
                </div>
              )}
            </TabsContent>
          </Tabs>
      ...
      {/* Task Detail Modal - Same structure as CoordinatorDashboard */}
      {selectedTask && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <Card className="w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <CardContent className="space-y-4">
              ...
              {/* Only show submission options for own tasks */}
              {myTasks.find(task => task._id === selectedTask._id) && selectedTask.status !== 'SUBMITTED' && (
                <>
                                ...
                                >
                                  {uploadedFiles[index] ? (
                                    ...
                                    <div className="flex items-center gap-2">
                                      ...
                                      <span className="text-sm">Upload {deliverable.type} file</span>
                                    </div>
                                  )}
                                </label>
                              </div>
